...
// Same stacking approach as EnergyMixChart
...
// Single stackId → one full-width stacked bar per month
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-lg text-sm min-w-[180px]">
        ...
        {prodItems.map((item) => (
          <div key={item.dataKey} className="flex items-center justify-between gap-3 mb-0.5">
            ...
            <span className="text-xs font-mono">{item.value.toFixed(1)}</span>
          </div>
        ))}
        <div className="border-t border-gray-100 mt-2 pt-1 space-y-0.5">
          <div className="flex justify-between text-xs">
            <span className="text-gray-500">Produzione</span>
            <span className="font-mono font-semibold">{totalProd.toFixed(1)} TWh</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-gray-500">Domanda</span>
            <span className="font-mono">{demandVal.toFixed(1)} TWh</span>
          </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-700">Produzione mensile e domanda (TWh)</h3>
        <span className="text-xs text-gray-400">Clicca su un mese per il dettaglio</span>
      </div>
      <ResponsiveContainer width="100%" height={280}>
        ...
        >
          ...
          <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} axisLine={false} tickLine={false} unit=" TWh" width={52} />
